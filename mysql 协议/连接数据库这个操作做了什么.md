## 概要
在 Python 语言环境下我们可以使用如下代码来连接数据库。
```python
...
## 连接的背后
当我们通过驱动程序(mysql-connector-python,pymysql)连接 MySQL 服务端的时候，驱动程序会发起到 MySQL 服务端的 TCP  连接。

当 TCP 连接建立之后驱动程序与服务端之间会按特定的格式和次序交换数据包，MySQL 协议就规定了以何种格式和次序来交换数据；下图是一个连接(登录)操作 C/S 所要交换的数据包。


![mysql-handshake](static/2020-22/mysql-handshake.png)

也就是说连接数据库就是，应用程序把连接参数传递给驱动程序，再由驱动程序建立与 MySQL-Server 的 TCP 连接。驱动程序还有一个功能就是把从 MySQL-Server 收到的字节流“翻译”成客户端语言能理解的数据结构。
...
## 祼写 TCP 完成 MySQL 的登录操作
从上面给出的信息可以看出像 mysql-connector-python,pymysql 这类的驱动程序，并不是什么神仙、皇帝，只是一个普普通通的 TCP 客户端。

战略上已经完成了对它的藐视，我用纯 TCP 编程实现了登录 MySQL，下面看一下没有驱动程序登录 MySQL 是一件多艰苦的事。
...
在不用连接驱动的情况下 300 多行代码才实现连接到 MySQL 这个操作。
