...
其实上面的解决方案就是一个典型的事件驱动式的任务处理，“举手” 就是事件，老师收到这个事件之后就知道“哦，这个同步会背了，去验收一下”。事件驱动的处理方式是一个比较高效的处理方式。 还有其它的处理方式吗？还真的有。
...
## epoll 的工作模式
事件驱动程序通常用于 IO 密集型应用的服务端，用我们刚才的例子来对比，老师就是一个服务端，学生就是客户端，举手就是事件。那么关于举手就有两种不同的模式。第一种 只要某个同学他会背诵了就直接举着手，永不放下，直接老师来验收他。 第二种 会背的同步只举手一下，然后就把手放心了(想想如果当时老师没有看到他就GG了)。 在 epoll 中第一种模式叫 Level-Trigger 这个也是默认的模式；第二种模式叫 Edge-Trigger 。
